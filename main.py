...
while input1!= "2":
    ...
    try:
        ...
        print("图片格式不正确！请重新选择图片！")
        continue

    
    filename = os.path.basename(pic1name)
    select += 1
    ...
    for file in files:
        ...
        if file.endswith(".png") or file.endswith(".jpg") or file.endswith(".jpeg"):
            pic3=file
            #print("文件名为："+pic3)
            #pic2=Image.open(r"C:\Users\Administrator\Downloads\ppic\零售链接"+"\\"+file)
            try:
    ...
    #输出差异的平均值，值越大差异越大
                ...
                print("!Warning:"+" "+"\""+file+"\""+"图片格式不正确！已跳过此图片")
                wrongpic += 1
                dealfiles +=1
    pbar.close()
    if dealfiles == filenum and foundup >= 1:
        print("\n抱歉没找到其他图片了，呜呜呜...\n")
        if wrongpic > 0:
            print("tip: 共有"+str(wrongpic)+"张图片格式不正确！已被跳过~\n")
    if dealfiles == filenum and foundup < 1:
